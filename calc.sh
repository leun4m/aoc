...
function year() {
    YEAR_DIR=$1
    YEAR=${YEAR_DIR/.\/src\/solutions\/year_//}
    YEAR=${YEAR/\//}
    PROGRESS=$(find "${YEAR_DIR}"/day_* 2>/dev/null | wc -l | sed -e 's/^[[:space:]]*//')
    PERCENT=$(awk "BEGIN{print ${PROGRESS}/25*100}")
